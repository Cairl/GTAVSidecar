...
```
GTAVSidecar/
├── main.py              # 框架入口 (事件循环 + TUI + anti_afk 管理, ~217行)
├── config.json          # 配置文件 (支持热重载 + 持久化)
├── .gitignore
├── core/                # 共享核心模块 (~1620行, 从 main.py 拆分)
│   ├── __init__.py      # 统一导出 + _INJECT_SYMBOLS 组装
│   ├── i18n.py          # 翻译引擎
│   ├── log_buffer.py    # 日志缓冲 + hack 显示状态
│   ├── config.py        # 配置管理 + Steam 语言检测 + 任务模块加载
...
│   └── show_performance/
│       └── task.py      # Task(BaseTask) 性能显示开关 (纯声明式)
└── .agents_logs/
```
...
```
focus_game_window(hwnd) -> bool      # 统一窗口聚焦, core/windows_api.py
bring_to_foreground(hwnd) -> bool    # 底层原语, core/windows_api.py
```

- `focus_game_window`: 统一入口，`bring_to_foreground` → 0.3s 等待 → 点击窗口中心 → 0.5s 等待
- `bring_to_foreground`: 循环重试最多 10 次，通过 `AttachThreadInput` + `SetForegroundWindow` 切换前台
- 所有需要游戏输入焦点的场景统一使用 `focus_game_window`，替代了旧的分散模式（`_click_matcher` 的 3s 条件等待、`execute_key_sequence` 的点中心激活、hack 任务的裸 `bring_to_foreground`）
- SendInput 只对前台窗口有效
...
**前台切换**: 所有点击/按键操作前统一调用 `focus_game_window(hwnd)` — `bring_to_foreground` → 0.3s → 窗口中心点击 → 0.5s，不再区分前后台场景。
...
- 分组标题不可选中，导航时自动跳过
- **性能显示开关**: 「显示当前占用性能」作为任务列表中的可切换项，开启时面板底部显示 CPU/内存行（左对齐），关闭时隐藏以节省取样开销
- 面板宽度由任务项内容决定，不受性能行影响
...
- **任务显示名**: 通过 `task.{key}` 翻译键映射，如 `task.create_invite_only` → `创建仅限邀请战局` / `Create Invite-Only Session`
- **分组标题**: 通过 `group.{group_key}` 翻译键映射，如 `group.hack_solver` → `黑客游戏自动破解`

**序列日志翻译**:

- `sequence_starting` / `sequence_done` 支持任务后缀变体（如 `sequence_starting.create_invite_only` → `开始创建`），未定义后缀 key 时自动降级到通用文本
...
**第一版解决**: 在 `_click_matcher` 中检测窗口是否已在后台，若从后台切回则等待 3 秒让游戏画面稳定后再点击。

**最终解决 (26w18e)**: 统一为 `focus_game_window()` — `bring_to_foreground` → 0.3s 等待 → 窗口中心点击 → 0.5s 等待，不再区分前后台场景。点中心能可靠激活游戏输入焦点，0.8s 总延迟兼容前后台切换。
...
| 字段                                     | 说明                                                                             |
| -------------------------------------- | ------------------------------------------------------------------------------ |
| `lang`                                 | 界面语言，`"auto"` 自动检测 Steam 游戏语言，可手动设 `zh_CN` / `zh_TW` / `en_US`      |
| `scan_ms`                              | 全局默认扫描间隔 (ms)                                                         |
| `anti_afk.enabled`                     | 防挂机功能开关                                                                       |
| `anti_afk.key`                         | 防挂机发送的按键 (默认 `enter`)，支持字母键、数字键、功能键等                                  |
| `anti_afk.interval_min`                | 防挂机按键间隔 (默认 10 分钟)                                                          |
| `tasks.<key>.enabled`                  | 任务启用/关闭                                                                       |
| `tasks.<key>.scan_ms`                  | 任务级扫描间隔                                                                        |

**配置职责分离**: `start_trigger`、`steps`、`group` 等任务结构定义在 `task.py` 的 `Task(BaseTask)` 类属性中；`config.json` 仅存储运行时参数（启停状态、检测间隔等），支持热重载。

**分组配置**: 任务的 `group` 在 `task.py` 的 `Task` 类属性中定义（如 `group = "hack_solver"`），对应 locale 翻译键 `group.hack_solver`。

**任务顺序**: 由 `core/config.py` 中的 `_TASK_ORDER` 列表统一控制菜单显示顺序和配置生成顺序。
...
7. 配置由 `_build_default_config()` 自动生成，无需手动编辑 config.json
...
12. task.py 中可直接使用 `BaseTask`、`OverlayMatcher`、`click_at` 等注入符号（来源于 `core/__init__.py` 的 `_INJECT_SYMBOLS`），标准库和第三方库需显式 import
13. 纯按键序列任务只需声明 `start_trigger = {}` 和带 `delay` 字段的 `steps`，框架自动调用 `execute_key_sequence`（参考 `create_invite_only/task.py`）
...
- `bring_to_foreground` 在某些情况下可能失败 (Windows 前台锁定策略)，`focus_game_window` 不依赖其返回值仍会执行窗口中心点击
...
**修改范围**: 架构重构 — `main.py` 拆分为 `core/` 共享模块；窗口聚焦统一为 `focus_game_window`；新增性能显示开关任务

**修改原因**: 原 `main.py` 1817 行单体文件庞大难以维护；窗口聚焦逻辑分散在 4 处（`_click_matcher` 的条件 3s 等待、`execute_key_sequence` 的点中心激活、hack 任务的裸调用、anti_afk 的裸调用）；性能显示常驻无法关闭

**修改内容**:

- **架构拆分**: 从 `main.py` 抽取 8 个独立模块到 `core/`：`i18n.py`、`log_buffer.py`、`config.py`、`windows_api.py`、`resource_monitor.py`、`task_base.py`、`task_runner.py`、`renderer.py`；`core/__init__.py` 统一导出 + `_INJECT_SYMBOLS` 组装；`main.py` 精简为 217 行事件循环
- **窗口聚焦统一**: 新增 `focus_game_window(hwnd)` — `bring_to_foreground` → 0.3s → 窗口中心点击 → 0.5s；替换 `_click_matcher` 中的 `was_foreground` 判断 + 3s 等待、`execute_key_sequence` 中的重复聚焦逻辑、hack 任务和 anti_afk 中的裸 `bring_to_foreground` 调用
- **性能显示任务**: 新建 `tasks/show_performance/task.py`（声明式伪任务），菜单中作为可切换项出现；开启时面板底部显示 CPU/内存行（左对齐），关闭时跳过 `sample_process_resources()` 节省开销；`_flatten_task_configs` 自动检测磁盘新增任务目录
- **序列日志定制**: `execute_key_sequence` 支持任务后缀翻译键（如 `sequence_starting.create_invite_only` → `开始创建`），未定义时降级到通用文本
- **`_TASK_ORDER`** 移至 `core/config.py`，新增 `show_performance` 项
- **翻译更新**: `zh_CN`/`zh_TW`/`en_US` 三语言新增 `sequence_starting`/`sequence_done` 及其 `create_invite_only` 变体、`task.show_performance`、`game_window_not_found`
- **踩坑 #6**: `bring_to_foreground` 失败时 `focus_game_window` 不提前返回，仍执行窗口中心点击

**修改前后行为差异**:

| 场景 | 修改前 | 修改后 |
|------|--------|--------|
| 代码结构 | `main.py` 1817 行单体 | 10 个聚焦模块（`core/` 下 8 个 + `main.py` 217 行） |
| 窗口聚焦 | 4 处不同实现 | 统一为 `focus_game_window` |
| 性能显示 | 常驻面板底部 | 菜单中可开关的「显示当前占用性能」任务 |
| 序列日志 | 通用"开始执行序列..."/"序列完成" | 任务定制日志（如"开始创建"/"创建完成"） |
| 性能行对齐 | 右对齐 + 撑宽面板 | 左对齐 + 不撑宽面板 |

**对系统影响范围**: 全面架构重构，所有模块归属变更，所有窗口切换逻辑变更；任务检测和执行逻辑不变；`config.json` 由 `_build_default_config()` 自动迁移
