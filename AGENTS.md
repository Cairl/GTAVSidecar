...
- 第三任务（IP 破解）: OCR 数字识别 + 上下左右箭头键路径规划
...
- 扫描码映射: `W=0x11, A=0x1E, S=0x1F, D=0x20, Enter=0x1C, Up=0x48, Down=0x50, Left=0x4B, Right=0x4D`
...
- **目标 IP 读取**: 连通组件检测 → 缩放到模板尺寸 → 形状匹配识别每个数字
...
```
1. 前台切换 + 鼠标锁定
2. 截屏 → OCR 读取目标 IP（4 组两位数，日志中红色显示）
...
IP 破解游戏有时间限制和错误次数限制，超时或错误过多会出现失败界面。失败界面通过 `fail.png` 覆盖图检测（按语言分目录），检测到后按 Enter 重置游戏并重新开始破解。
...
| 目标 IP 读取失败 + 失败界面存在 | 初始扫描 | 先 `_check_fail`，失败则 `"reset"` |
...
2. TUI 网格面板有 "目标: XX.XX.XX.XX" header 行，与日志区 "目标 IP: XX.XX.XX.XX" 重复
...
- 移除 `_build_grid_panel` 的 "目标:" header 行，目标 IP 已在日志区红色显示
...
- 目标 IP 日志行中的数字使用 `C_RED` 颜色高亮，与游戏内红色目标 IP 视觉一致
